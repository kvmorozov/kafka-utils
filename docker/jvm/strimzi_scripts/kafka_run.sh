...
KAFKA_OPTS="${KAFKA_OPTS} -XX:SharedArchiveFile=/opt/kafka/storage.jsa"
...
KAFKA_OPTS="${KAFKA_OPTS} -javaagent:$(ls "$KAFKA_HOME"/libs/kafka-agent*.jar)=/tmp/kafka-agent.properties -XX:SharedArchiveFile=/opt/kafka/kafka.jsa"
export KAFKA_OPTS
